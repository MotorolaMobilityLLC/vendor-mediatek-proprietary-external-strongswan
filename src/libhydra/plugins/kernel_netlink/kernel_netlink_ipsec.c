		...
		mark_t mark, policy_priority_t prio)
{
	...
	/* Try delete again as action block policy left sometimes (kernel 4.14 and above), netgen deletes sometimes, 
	* It is ok to delete (as per kernel owner) because if there is no policy then it will just fail.
	*/
	if (this->socket_xfrm->send_ack(this->socket_xfrm, hdr) != SUCCESS)
	{
		DBG1(DBG_KNL, "delete policy tried to make sure block policy not left");
	}
